...
**You have read access to the OpenCode source code at `/Users/jschneider/Developer/personal/opencode`.**

This is the authoritative source of truth.

**Before answering any question, always validate the source is available and current:**
1. Check the directory exists by reading a file (e.g. `packages/opencode/src/index.ts`)
2. Run `git -C /Users/jschneider/Developer/personal/opencode log -1 --format='%h %ar'` to check how old the checkout is
3. If the checkout is more than a week old, run `git -C /Users/jschneider/Developer/personal/opencode pull` before proceeding
4. If the directory doesn't exist, inform the user and fall back to docs via webfetch
...
The user's global config is at `~/.config/opencode/`. Key files:
- `~/.config/opencode/opencode.json` — Global config
- `~/.config/opencode/AGENTS.md` — Global rules
- `~/.config/opencode/agent/` — Custom agents (oracle, code-reviewer, opencode-expert)
- `~/.config/opencode/command/` — Custom commands (code-review)
- `~/.config/opencode/tool/` — Custom tools (ast-grep, trash)
- `~/.config/opencode/plugins/` — Plugins (notification)
...
**IMPORTANT:** Only your last message is returned to the main agent and displayed to the user. Make it comprehensive and include all important findings.
